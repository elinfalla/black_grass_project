...
* Farm management changes (variable name = "man_strats") - binary data on how the farm management strategies affect the species' resources  
...
mammals <- read_xlsx("Mammal_requirements.xlsx", skip = 2) 
...
``` {r, purl=FALSE}
calculate_A_B(mammals, man_strats[1,])
```
...
out <- data.frame(species = all_species,
                  Dt = calculate_Dt(mammals, man_strats[2,]))
out
```
...
``` {r test-nt, purl=FALSE}
out <- data.frame(species = all_species,
                  Nt = calculate_Nt(mammals, man_strats[1,]))
out
```
...
### 5.1. Test risk scores are correct
...
riskscores <- calc_mammal_riskscore(mammals, man_strats)
...
butler$my_risk == butler$butler_risk *-1
```
