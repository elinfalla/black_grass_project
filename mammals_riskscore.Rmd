...
calculate_A_B <- function(data, strat) {
  ...
  hab_cols <- ncol(diets_habs %>% select(hab_start : hab_end))
  last_d_col <- ncol(diets_habs) - num_hab_cols
  
  # check requirements data and management change data align
  check_aligned(colnames(diets_habs), colnames(strat))
  
  # find overlap between diet + hab requirements and management change
  overlap <- data.frame(t(apply(diets_habs, 1, function(row) row & strat))) 
