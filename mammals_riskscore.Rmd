...
#mammals <- mammals[mammals$Species...1 == "Harvest mouse" | mammals$Species...1 == "Wood mouse",]
...
calculate_A_B <- function(data, strat) {
  ...
  # PUT IN CHECK HERE THAT COLUMNS ALIGN
...
calculate_C1_C2 <- function(data, strat) {
  ...
  C1_overlap <- t(apply(nesting, 1, function(row) row & nest_success))  # calculate overlap
  ...
  C2_overlap <- t(apply(nesting, 1, function(row) row & nest_habs))  # calculate overlap
...
calculate_riskscores <- function(data, strategies) {
  ...
  # CALCULATE RISK SCORE for each strategy and put into riskscores dataframe
...
# Find total risk for each species (sum up all risks)
riskscores$total_risk <- rowSums(riskscores[,-1])

# Display riskscores
riskscores
riskscores[riskscores$species == "Harvest mouse",]
```
