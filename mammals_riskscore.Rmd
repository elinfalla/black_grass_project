...
calculate_A_B <- function(data, strat) {
  ...
  strat <- strat %>%
    select(C_BG : R_hab) %>%
    select(!c(C_AQ, M_AQ, H_AQ)) # THIS MAY CHANGE - remove AQ columns (except R_AQ) so cols align
...
calculate_Nt <- function(data, strat) {
  ...
  calc_Nt <- cbind(N, C1_C2)
  
  Nt <- (calc_Nt$C1 / calc_Nt$N) + (calc_Nt$C2 / calc_Nt$N)
...
calculate_riskscores <- function(data, strategies) {
  ...
  #colnames(riskscores) <- as.character(strategies[1,]) # TODO
...
``` {r test-risk}
risks <- calculate_riskscores(mammals, man_strats[1,])
risks
total_risks <- rowSums(risks[,-1])
total_risks
```
