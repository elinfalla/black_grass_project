...
if (input == "butterflies" | input == "both") {
  ...
  butterfly <- read_xlsx("../data/Butterfly_requirements.xlsx", skip = 3)
  
  # forage plant locations
  butt_plant_locs <- read_xlsx("../data/forage_plant_location.xlsx", na = "NA")
  
  # butterfly forage plants
  butterfly_plants <- read_xlsx("../data/butterfly_forage_plants.xlsx")
...
if (input == "butterflies" | input == "both") {
  ...
  butt_man_strats <- read_xlsx("../data/change.xlsx", sheet="butterflies")
...
if (input == "bees" | input == "both") {
  ...
  bee <- read_xlsx("../data/Pollinator_requirements.xlsx") 
  ...
  # forage plant locations
  bee_plant_locs <- read_xlsx("../data/forage_plant_location.xlsx", na="NA")
...
if (input == "bees" | input == "both") {
  ...
  bee_man_strats <- read_xlsx("../data/change.xlsx", sheet="bees", skip=1) 
...
If one wants to calculate standardised risk scores from scratch (i.e. calculate risk scores then standardise them), the 'data' argument needs to be the species' requirements dataset, 'strategies' needs to be the management strategies dataset, and 'plants' and 'plant_locs' need to be the species' forage plants and their locations respectively. 'from_scratch' must be set to TRUE, and the 'risk_col' argument can be ignored.  

If one already has a dataset with risk scores in it that need to be standardised, 'data' needs to be said dataset, and 'risk_col' needs to be the name of the column with the risk scores in it. 'from_scratch' is FALSE by default and the 'strategies', 'plants', and 'plant_locs' arguments can be ignored.

``` {r calc-standardised-butt-riskscore}
calc_std_butterfly_riskscore <- function(data, plants=NA, plant_locs=NA, strategies=NA, from_scratch=FALSE, risk_col=NA) {
...
``` {r calc-standardised-bee-riskscore}
calc_std_bee_riskscore <- function(data, plants=NA, plant_locs=NA, strategies=NA, from_scratch=FALSE, risk_col=NA) {
...
if (input == "bees" | input == "both") {
  ...
  bee_butler_risk <- read_xlsx("../data/Butler_validation/Pollinator risk assessment database.xlsx", 9)
