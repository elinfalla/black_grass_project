...
test G + H func:
``` {r test-GH}
res <- calculate_Ga_Gl_Ha_Hl(butterfly)
...
calculate_Gta_Gtl <- function(data, strat) {
  ...
  # gens_affected <- periods_affected * data.frame(early, mid, late)
  # output[,i] <- rowSums(gens_affected)
  
  output[,i] <- rowSums(all_gens_affected)
...
test calc Gt:
``` {r test-Gt}
out <- calculate_Gta_Gtl(butterfly, man_strats[3,])
...
Butterflies have long tongues, so adults can forage on any plant in the habitat(s) they occupy. Therefore, Fa (F for adults) will just be the total number of forage plants that are found in the species habitat(s) that all butterflies will forage on.
...
sum <- 0
for (i in 1:6) {
  sum <- sum + calculate_Fta_Ftl(butterfly, butterfly_plants, plant_locs, man_strats[i,])
  sum <- sum + calculate_Pta_Ptl(butterfly, man_strats[i,])
}
print(sum)
...
butler_sorted$same_BEFORE <- butler_sorted$same
...
butler_sorted$same <- butler_sorted$butler_risk == butler_sorted$wo_cropped
butler_sorted$diff <- butler_sorted$butler_risk - butler_sorted$wo_cropped


butler_sorted$new <- round(risk$total_risk, 2)
print(round(risk$total_risk, 2))
