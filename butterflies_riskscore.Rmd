...
calculate_Fa <- function(data, plant_locs) {
  ...
  # for every species (ie. every row in habs dataframe)
  for (i in 1:nrow(habs)) {
    locs <- plant_locs[,which(habs[i,] == 1)] # extract cols of plant_locs for which butterfly habs=1
    
    if(ncol(locs) > 1) {  # if species has more than one habitat

      col <- locs[,1]  # set first column as current column
      
      # loop over columns, applying or operator successively, to find all plants species can forage on
      for (j in 2:ncol(locs)) {  # start from second column
        col <- col | locs[,j]
      }
    ...
    else {  # if plant locations has a single column (ie. species found in 1 hab)
      Fa[i] <- sum(locs)  # Fa is total number of plants
    }
...
``` {r test-Fa}
calculate_Fa(butterfly, plant_locs)
```