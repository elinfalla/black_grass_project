...
calculate_A_F <- function(data, plants, plant_locs, strat) {
  ...
  for(species in 1:nrow(habs)) {
    ...
    for (effect in 1:length(sp_effects)) {
      ...
      else if (hab_effects$CG_hab != 0 & hab_effects$CA_hab != 0){ #  if strat affects all crops (not damp???)
...
calculate_N_Hn <- function(data) {
  ...
  nest_sites <- c("Versatile", "Aerial", "Bare", "Extensive", "Open", "Snail")
  N <- 0
...
if (input == "bees" | input == "both") {
  ...
  bee_butler_risk <- read_xls("Butler_validation/Pollinator risk assessment database.xls", 9)
