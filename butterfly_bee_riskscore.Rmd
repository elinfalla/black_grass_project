...
## 2.4. Create function to check column alignment
...
calculate_Gt <- function(data, strat) {
  ...
  check_aligned(activity, activity_effects)
...
if (input == "bees") {
  ...
  dat <- data.frame(species = all_species,
                    Pt = calculate_Pt(bee, bee_man_strats[4,]))
  dat
...
calculate_A_F <- function(data, plants, plant_locs, strat) {
  ...
  check_aligned(habs, hab_effects)
...
if (input == "bees") {
  ...
  dat <- data.frame(species = all_species,  
                    calculate_A_F(bee, bee_plants, bee_plant_locs, bee_man_strats[6,]))
  dat
...
if (input == "bees") {
  out <- data.frame(species = all_species,
                  Ft = calculate_Ft(bee, bee_plants, bee_plant_locs, bee_man_strats[4,]))
...
calculate_B <- function(data, strat) {
  ...
  # check requirements and change data columns are aligned (i.e. the same)
  check_aligned(nesting, nesting_effects)
...
calculate_Nt <- function(data, strat) {
  ...
  N_times_Hn <- calculate_N_Hn(data)
...
``` {r test-sums, purl=FALSE}
if (input == "bees") {
  out <- calc_bee_riskscore(bee, bee_plants, bee_plant_locs, bee_man_strats)
  out
  # bombus_out <- out %>%
  #   filter(grepl("Bombus", species))
  # bombus_out
