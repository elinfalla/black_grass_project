...
calculate_Nt_Mt <- function(data, strat) {
  ...
  # calculate E and Nt for each species (nitrogen affinity)
  # calc_Nt <- nitrogen %>%
  ...
  # calculate M and Mt for each species (moisture affinity)
  # calc_Mt <- moisture %>%
  ...
  # multiply Nt/Mt by overlaps to take man strat into account
  Nt <- rowSums(nitro_overlap) * Nt
  Mt <- rowSums(moist_overlap) * Mt
