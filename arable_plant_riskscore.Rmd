...
dat <- data.frame(species = all_species,
           Gt = calculate_Gt(plants, man_strats[1,]))
nrow(dat[dat$Gt != 0,])
dat
...
dat <- data.frame(species = all_species,
                  Ft = calculate_Ft(plants, man_strats[2,]))
#dat$Ft <- add_condition(plants, dat$Ft, "_Au")  # add the condition that the species germinates in autumn to Ft

dat[dat$Ft != 0, "Ft"] == plant_validation[plant_validation$`Reduced crop flowering` != "Geranium pusillum", "Score...3"]
dat[dat$Ft != 0,]
```
...
calculate_Nt_Mt <- function(data, strat) {
  ...
  E <- data %>% # ellenberg nitrogen affinity score
    pull(Nitrogen)
  ...
  M <- data %>% # ellenberg moisture affinity score
    pull(Moisture)
  ...
  # calculate Nt for each species (nitrogen affinity)
  Nt <- (10 - E + 1) / 10
  #Nt_per_hab <- Nt / num_habs
  
  # calculate Mt for each species (moisture affinity), and divide by number of habitats to get per habitat score
  Mt <- M / 10
  Mt_per_hab <- Mt / num_habs
  ...
  # multiply Nt and Mt_per_hab by overlaps to take man strat into account
  Nt <- rowSums(nitro_overlap) * Nt
  Mt <- rowSums(moist_overlap) * Mt_per_hab
...
dat <- data.frame(species = all_species,
                  Nt = calculate_Nt_Mt(plants, man_strats[2,])$Nt)
dat[dat$Nt != 0,]
