...
### 2.3. Create function to check column alignment
It is essential to the method used here to ensure that the columns of the management change dataset and species' requirements dataset align before calculating overlap parameters - i.e. parameters that work out the impact of a change on a specific resource for a species. The below function checks this, and outputs a warning if the column names of the two inputted datasets are not identical.  

``` {r check-alignment}
check_aligned <- function(data1, data2) {
    
  aligned <- identical(colnames(data1), colnames(data2))
...
calculate_H_G_A <- function(data, strat) {
  ...
  check_aligned(germination, germination_effects)
...
calculate_H_F_B <- function(data, strat) {
  ...
  # check requirements and change data columns are aligned (i.e. the same)
  check_aligned(flowering, flowering_effects)
...
calculate_Nt_Mt <- function(data, strat) {
  ...
  check_aligned(nitrogen, nitrogen_effects)
  check_aligned(moisture, moisture_effects)
