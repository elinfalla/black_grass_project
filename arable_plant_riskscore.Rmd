...
man_strats <- read_xlsx("change.xlsx", 7, skip = 1)
...
This function calls ```calculate_H_G_A()``` then uses the output to calculate $G_t$ for each species.  
...
``` {r test-gt}
all_species <- plants$Species
dat <- data.frame(species = all_species,
           Gt = calculate_Gt(plants, man_strats[4,]))
nrow(dat[dat$Gt != 0,])
dat[80,]
butler <- read_xlsx("butler_plants_risk.xlsx")

butler$Gt_1 == dat[dat$Gt != 0, "Gt"]
```
...
This function calculates $F_t$ for each species by calling ```calculate_H_F_B()```.  
...
``` {r }
plant_validation <- read_xls("Butler_validation/Validation of weed matrix.xls", 7)

dat <- data.frame(species = all_species,
                  Ft = calculate_Ft(plants, man_strats[4,]))
#dat$Ft <- add_condition(plants, dat$Ft, "_Au")  # add the condition that the species germinates in autumn to Ft

dat[dat$Ft != 0, "Ft"] == plant_validation[, "Score"]
dat[dat$Ft != 0,]
dat[80,]
```
...
dat <- data.frame(species = all_species,
                  Nt = calculate_Nt_Mt(plants, man_strats[4,]))
dat[dat$Nt != 0,]
dat[80,]
calculate_Nt_Mt(plants, man_strats[2,])
...
### Make function for autumn sowing condition (one of Butler's management changes)
The 'autumn sowing' management change causes decline for July flowering crop habitat plants **only if** they are autumn germinating.  

I've therefore created a function that allows the addition of a condition to a parameter after calculating it. In this specific case, implemented in the ```calc_plant_risk()``` function below, I'm adding a condition that the species must be autumn germinating to $F_t$ (the effects on flowering).  
